...
# Forçar embed=true no Streamlit Cloud
st.markdown("""
<script>
(function(){
  try{
    ...
    if(url.hostname.endsWith("streamlit.app") && url.searchParams.get("embed")!=="true"){
      url.searchParams.set("embed","true"); window.location.replace(url.toString());
    }
  }catch(e){}
})();
</script>
...
# -------------------- Background + Overlay (mostra o mapa!) --------------------
...
if Path(bg_file).exists():
    ...
    /* imagem de fundo (mapa claro) */
    .stApp {{
      background-image: url('{bg_file}');
      background-size: cover;
      background-position: right center; /* globo mais à direita */
      background-attachment: fixed;
    }}

    /* overlay escuro por cima da imagem mas atrás do conteúdo */
    .stApp::before {{
      content:"";
    ...
    st.markdown("""
    <style>
    .stApp{ background: linear-gradient(140deg,#0f2d55 0%, #123a6e 45%, #144180 100%); }
    </style>
    """, unsafe_allow_html=True)
...
if auth_status:
    # mostra sidebar somente depois do login
    show_sidebar()
